...
This weighted blend of value vectors results in a vector that paid 50% of its “attention” to the word 
```plaintext
robot
```
, 30% to the word 
...
it
```
. Later in the post, we’ll got deeper into self-attention. But first, let’s continue our journey up the stack towards the output of the model.
...
Earlier in the post we showed this image to showcase self-attention being applied in a layer that is processing the word 
```plaintext
it
```
:
...
In this case, we process the first token (ignoring 
```plaintext
<s>
```
 for now).

GPT-2 holds on to the key and value vectors of the the 
```plaintext
a
```
 token. Every self-attention layer holds on to its respective key and value vectors for that token:

Now in the next iteration, when the model processes the word 
```plaintext
robot
```
, it does not need to generate query, key, and value queries for the 
```plaintext
a
```
 token. It just reuses the ones it saved from the first iteration:

#### GPT-2 Self-attention: 1- Creating queries, keys, and values

Let’s assume the model is processing the word 
```plaintext
it
```
. If we’re talking about the bottom block, then its input for that token would be the embedding of 
```plaintext
it
```
 + the positional encoding for slot #9:
...
[![Image 2: Creative Commons License](https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png)](http://creativecommons.org/licenses/by-nc-sa/4.0/)
